...
def start():
    while True:
        ...
        match choice:
            case 1:
                model.open_pb()
                model.print_message(text.load_successful)
            case 2:
                model.save_pb()
                model.print_message(text.save_successful)
            case 3:
                pb = model.get_pb()
            ...
            case 6:
                ...
                if result:
                    ...
                    name = model.change_contact(new_contact, current_id) # type: ignore
            ...
            case 7:
                ...
                if result:
                    if len(result) != 1:
                        view.print_contacts(result, '')
                        current_id = view.input_search(text.delete_index)
                    else:
                        current_id = result[0].get('id')
                    name = model.search_name(current_id)
                    renewed_pb = model.delete_contact(current_id) # type: ignore
                    view.print_message(text.delete_successful(name))
                    view.print_contacts(renewed_pb)
                else:
                    view.print_message(text.empty_search(key_word))
            case 8:
                break
